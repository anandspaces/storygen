...
const startVideoJobVertex = async ({ prompt, model, firstFrame, lastFrame, durationSeconds }) => {
  ...
  // ✅ Create auth with keyFilename and get the token
...
const pollOperationVertex = async (name, maxAttempts = 60, delayMs = 40000) => {
  ...
  // ✅ Add keyFilename for consistent authentication
  const auth = new GoogleAuth({ 
    keyFilename: process.env.GOOGLE_APPLICATION_CREDENTIALS || path.join(__dirname, '../../key.json'),
    scopes: ['https://www.googleapis.com/auth/cloud-platform'] 
  });
